...
class DenoiseModel(nn.Module):
    ...
    def forward(self, spad):
        ...
        ds1_out = self.ds1(spad)
        ds1_out.cpu().numpy().astype('float32').tofile('Mode_parameters/ds1out')
        ds2_out = self.ds2(ds1_out)
        ds2_out.cpu().numpy().astype('float32').tofile('Mode_parameters/ds2out')
        ds3_out = self.ds3(ds2_out)
        ds3_out.cpu().numpy().astype('float32').tofile('Mode_parameters/ds3out')

        conv0_out = self.conv0(spad)
        conv0_out.cpu().numpy().astype('float32').tofile('Mode_parameters/conv0out')
        conv1_out = self.conv1(ds1_out)
        conv1_out.cpu().numpy().astype('float32').tofile('Mode_parameters/conv1out')
        conv2_out = self.conv2(ds2_out)
        conv2_out.cpu().numpy().astype('float32').tofile('Mode_parameters/conv2out')
        conv3_out = self.conv3(ds3_out)
        conv3_out.cpu().numpy().astype('float32').tofile('Mode_parameters/conv3out')

        up3_out = self.up3(conv3_out)
        up3_out.cpu().numpy().astype('float32').tofile('Mode_parameters/up3out')
        up2_out = self.up2(torch.cat((conv2_out, up3_out), 1))
        up2_out.cpu().numpy().astype('float32').tofile('Mode_parameters/up2out')
        up1_out = self.up1(torch.cat((conv1_out, up2_out), 1))
        up1_out.cpu().numpy().astype('float32').tofile('Mode_parameters/up1out')
        up0_out = torch.cat((conv0_out, up1_out), 1)
        up0_out.cpu().numpy().astype('float32').tofile('Mode_parameters/up0out')

        refine_out = self.refine(up0_out)
        refine_out.cpu().numpy().astype('float32').tofile('Mode_parameters/refineout')
        regress_out = self.regress(refine_out)
        regress_out.cpu().numpy().astype('float32').tofile('Mode_parameters/regressout')
