...
#include "xmpool_layer_hw.h"

#define MAP_SIZE (1024UL*1024UL)

void hw_maxpool_layer(int target,             // control address in the system
                   ...
                   )         
{
  ...
  // Write arguments
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_INPUT_OFFSET_DATA, input_offset); 
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_OUTPUT_OFFSET_DATA, output_offset);
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_B_DATA, b);
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_OD_DATA, od);
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_OX_DATA, ox);
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_OY_DATA, oy);
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_ID_DATA, id);
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_IX_DATA, ix);
  write_int(map_base, target + XMPOOL_LAYER_CTRL_BUS_ADDR_IY_DATA, iy);

  char* in_buffer = NULL;
  char* allocated = NULL;
  // Size to DMA in
  int size = id*ix*iy*b;//id*od*k*k+od+id*ix*iy*b;
  int wait_time = 0;
