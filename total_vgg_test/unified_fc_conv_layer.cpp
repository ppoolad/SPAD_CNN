...
void unified_fc_conv_layer(float * mem,            // global memory pointer
...
//   myDataType sumArrx[MAX_KERNEL_SIZE*MAX_KERNEL_SIZE]={0.0};
// #pragma HLS ARRAY_PARTITION variable=sumArrx complete dim=0
  myDataType sumArrz[CONV_MAX_KERNEL_SIZE][NUM_ELEM]={0.0};
#pragma HLS ARRAY_PARTITION variable=sumArrz cyclic factor=32 dim=2
//   myDataType bias_buf[MAX_OUTPUT_DIMS];
// #pragma HLS ARRAY_PARTITION variable=bias_buf cyclic factor=32
// static myDataType batch_out[MAX_BATCH] = {0};
// #pragma HLS ARRAY_PARTITION variable=batch_out complete
static float obf[MAX_BATCH][CONV_MAX_OUTPUT_WIDTH];
static float kbuf[FC_MAX_INPUT_SIZE][CONV_MAX_KERNEL_SIZE*CONV_MAX_KERNEL_SIZE]={0.0};
#pragma HLS ARRAY_PARTITION variable=kbuf cyclic factor=32 dim=1
static myDataType ibuf[FC_MAX_INPUT_SIZE][CONV_MAX_KERNEL_SIZE]={0.0};
...
#pragma HLS LOOP_TRIPCOUNT max=512
             ...
             // static float kbuf[MAX_INPUT_DIMS][MAX_KERNEL_SIZE*MAX_KERNEL_SIZE];
...
#pragma HLS PIPELINE
                          for(int q = 0; q < 64; q++)
                          {
                            sumArrz[iix][q] = ibuf[64*i_d+ q][iix] * (myDataType)kbuf[64*i_d+q][iiy*k  + iix];
                          }
                          output_element += hls_fp_accumulator(sumArrz[iix]);
                          //sumd2[i_d] = hls_fp_accumulator(sumArrz[iix]);
                  ...
                  if(relu){
                    if(output_element<0.0)output_element=0;}
                  obf[b_][o_x] = (float) output_element;
                }
              }
...
// #pragma HLS ARRAY_PARTITION variable=window7 complete
	...
	L5: for(int x=0; x<NUM_ELEM/32; x++)
    {
    	 window5[x] = window4[x] +  window4[NUM_ELEM/32+x];
	}

	result = window5[0] + window5[1];
	return result;
}

//
//static myDataType hls_fp_accumulator16(myDataType window0[NUM_ELEM2])
//{
//#pragma HLS PIPELINE
//	myDataType window1[NUM_ELEM2/2];// = {0.0};
//#pragma HLS ARRAY_PARTITION variable=window1 complete
////#pragma HLS RESOURCE variable=window1 core=FAddSub_nodsp
//	myDataType window2[NUM_ELEM2/4];// = {0.0};
//#pragma HLS ARRAY_PARTITION variable=window2 complete
//	myDataType window3[NUM_ELEM2/8];// = {0.0};
//#pragma HLS ARRAY_PARTITION variable=window3 complete
////	myDataType window4[NUM_ELEM2/16];//= {0.0};
////#pragma HLS ARRAY_PARTITION variable=window4 complete
//// 	myDataType window5[NUM_ELEM2/32];//= {0.0};
//// #pragma HLS ARRAY_PARTITION variable=window5 complete
//
////	myDataType window6[NUM_ELEM/64];//= {0.0};
////#pragma HLS ARRAY_PARTITION variable=window6 complete
...
//
//	myDataType result = 0.0;
//	L1: for(int x=0; x<NUM_ELEM2/2; x++)
//    {
//    	 window1[x] = window0[2*x] +  window0[1+2*x];
//	}
//	L2: for(int x=0; x<NUM_ELEM2/4; x++)
//    {
//    	 window2[x] = window1[x] +  window1[NUM_ELEM/4+x];
//
//	}
//	L3: for(int x=0; x<NUM_ELEM2/8; x++)
//    {
//    	 window3[x] = window2[x] +  window2[NUM_ELEM/8+x];
//
//	}
//	result = window3[0] + window3[1];
//	return result;
//}
