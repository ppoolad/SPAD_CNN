...
int run_conv(int numBatches)
{
  ...
  if(readInputBatches(imageRootDir, batch_layer_params, numBatches, layer, CONV_MAX_WEIGHT_SIZE+CONV_MAX_OUTPUT_DIMS+MAX_BATCH*CONV_MAX_CONV_INPUT+MAX_BATCH*CONV_MAX_CONV_OUTPUT, dma_input_vec, CONV))
	return 1;
  if(readOutputBatches(imageRootDir, batch_layer_params, numBatches, layer, MAX_BATCH*CONV_MAX_CONV_OUTPUT, gold_outputs_vec, CONV)) return 1;
